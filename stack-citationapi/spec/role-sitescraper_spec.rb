...
describe 'stack-citationapi::role-sitescraper' do
  ...
  let(:template_name) { '/etc/nginx/sites-enabled/sitescraper.conf' }
  ...
  describe 'deployment' do
    ...
    describe 'virtualhost' do
      ...
      it "writes virtualhost for api 'data'" do
        ...
        expect(template_resource).to notify('execute[nginx_configtest_sitescraper]')
          .to(:run)
          .immediately
      end
      ...
      it 'sets the correct root' do
        expect(chef_run).to render_file(template_name)
          .with_content(
            include('root /srv/www/sitescraper/current/public/;')
          )
      end
      ...
      it 'sets the correct upstream' do
        expect(chef_run).to render_file(template_name)
          ...
          .with_content(
            include('upstream sitescraper_phpfpm {')
          )
      end
